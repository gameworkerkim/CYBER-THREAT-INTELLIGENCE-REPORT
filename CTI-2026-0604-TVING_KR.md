...
![티빙 인앱 개인정보 유출 통지 팝업 — 닫기 버튼 없이 "다시 보지 않기"만 제공된다](images/CTI-2026-0604-TVING-popup.png)
...
| 일시 | 사건 | 비고 |
| --- | --- | --- |
| 2026-06-01 | 티빙, 과학기술정보통신부에 침해사고 신고 | 최초 인지 추정 시점 |
| 2026-06-02 | 회사 공지 기준 "유출 정황 확인" | 전체 범위 파악 완료 시점으로 추정 |
...
과기부 신고(6/1)와 공지문상 "확인" 시점(6/2)의 불일치는 최초 인지와 범위 확정의 차이로 해석되나, 인지–신고–통지의 시계열은 72시간 통지 의무 준수 여부와 직결되므로 개인정보위 조사에서 정밀 검증돼야 할 대목이다.
...
### 3.4 MITRE ATT&CK 매핑 (추정)

| 단계 | 기법 | 비고 |
| --- | --- | --- |
| Initial Access | **T1190** Exploit Public-Facing Application 또는 **T1078.004** Valid Accounts: Cloud Accounts | 경위 미확정 — 두 경로 모두 "클라우드 접근 통제 정책 변경"과 정합 |
| Collection | **T1005** Data from Local System / **T1213** Data from Information Repositories | 개인정보 DB 파일 수집 |
| Exfiltration | **T1048** Exfiltration Over Alternative Protocol / **T1567** Exfiltration Over Web Service | 외부 전송 채널 미공개 |

경위가 공식 확정되지 않은 만큼 본 매핑은 가설 트리이며, 조사 결과 공개 시 갱신한다. 요컨대 이번 침해는 단일 취약점의 문제가 아니라 심층 방어(defense in depth)의 연쇄 부재다. 경계–이그레스–탐지 세 계층 중 어느 하나만 정상이었어도 유출은 차단되거나 조기에 절단됐을 것이다.
...
![티빙 대표이사 사과문 전문 (2026-06-03 게재, 6/4 캡처 시점 조회수 80,199)](images/CTI-2026-0604-TVING-apology.png)
...
![1차 측정 — 2026-06-04 21:43 고객센터 공지 목록 (통지 129,599 / 사과문 79,738)](images/CTI-2026-0604-TVING-views-2143.png)

*그림 3. 1차 측정 (2026-06-04 21:43). 유출 사실 통지 129,599 / 대표 사과문 79,738.*

![2차 측정 — 2026-06-04 21:55 고객센터 공지 목록 (통지 129,724 / 사과문 80,457)](images/CTI-2026-0604-TVING-views-2155.png)
...
5. **통지 인터페이스 가이드라인 제정** — 유출 통지의 최소 UX 요건(재노출 횟수, 영구 미노출 옵션 제한, 도달률 보고 의무)을 시행령 또는 고시 수준에서 규정한다.

### 이용자

6. **즉시 조치** — 티빙 및 동일 비밀번호 사용 서비스의 비밀번호 변경, 2단계 인증 활성화.
7. **지속 경계** — 이름·생년월일·전화번호를 정확히 아는 정밀 피싱(택배·환불·수사기관 사칭)을 기본값으로 경계한다. 유출 정보 기반 피싱은 통상 유출 수주~수개월 후 도착한다.
8. **피해 접수** — 티빙 CX팀(1551-2391, tving@cj.net), KISA 118, 개인정보침해 신고센터.
...
3. 데일리시큐 — "개인정보위, 티빙 개인정보 유출 사고 조사 착수" (2026-06-04)
4. 연합인포맥스 — "티빙, 개인정보 유출…이름·생년월일·온라인용 주민번호 CI까지 털렸다" (2026-06-03)
5. 쿠키뉴스 — "티빙, 회원 개인정보 유출…공격자 IP 접근 차단" (2026-06-03)
6. 스포츠경향 — "티빙, 대표 나서 개인정보 유출 사과" (2026-06-04)
7. 한국경제(2025-02)·딜사이트 — 티빙 유료 가입자·목표 수치 보도
8. 나무위키 — "티빙 개인정보 유출 사건" (타임라인·KT 보상 이용자 관련, 비공식 출처로 교차 검증 필요)
9. 개인정보보호법 제34조 및 동법 시행령 (유출 통지 요건)
