...
- **위협 ① — LiteSpeed cPanel 플러그인 RCE (CVE-2026-48172, CVSS 10.0):** 공유 호스팅에서 모든 cPanel 사용자(공격자·탈취 계정 포함)가 `lsws.redisAble` JSON-API 한 번의 호출만으로 root 권한 임의 스크립트 실행이 가능하다. 인증 우회·레이스 컨디션이 불필요하며 **이미 야생에서 활발히 악용 중**이다.
